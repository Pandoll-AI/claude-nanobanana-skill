...
- `{MODEL}`: `fast` | `thinking` | `pro` (필수 — 아래 기준으로 직접 선택할 것)

### 모델 선택 기준 (Claude가 반드시 판단하여 지정)

| 모델 | 언제 사용 | 예시 |
|------|----------|------|
| **pro** | 행동·로직·게임·구조적 사고가 필요한 이미지. 캐릭터 동작 시퀀스, 스프라이트 시트, 상태별 포즈, 다이어그램, UI 목업, 멀티 패널 레이아웃, 정확한 공간 배치가 필요한 경우 | "8방향 걷기 스프라이트", "게임 캐릭터 공격 모션 시퀀스", "앱 UI 와이어프레임" |
| **thinking** | 세밀한 묘사·아트 스타일·분위기가 핵심인 이미지. 사실적 렌더링, 특정 화풍, 조명/구도 지정, 긴 디테일 프롬프트 | "oil painting of a medieval castle at golden hour with dramatic clouds", "studio ghibli style 풍경화" |
| **fast** | 간단한 요청, 빠른 프로토타이핑, 아무 이미지나 랜덤, 짧은 프롬프트, 스타일 미지정 | "a cute cat", "sunset", "아무 이미지나 만들어줘" |

**판단 우선순위**: 구조/행동 → pro, 묘사/아트 → thinking, 나머지 → fast.
사용자가 모델을 명시하면 그대로 따를 것. `auto`는 fallback으로만 사용 (키워드 자동 분류, 정확도 낮음).
