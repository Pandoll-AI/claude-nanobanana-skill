...
def remove_watermark(image_path: str | Path, output_path: str | Path | None = None) -> dict:
    ...
    # NCC snap search: 예상 위치 주변 ±2px에서 최적 위치 탐색
    alpha_2d = alpha_map[:ls, :ls]
    img_gray = np.mean(img_array, axis=2)
    alpha_flat = alpha_2d.astype(np.float64)
    best_ncc, best_dx, best_dy = -1, 0, 0
    for dy in range(-2, 3):
        for dx in range(-2, 3):
            yy, xx = y + dy, x + dx
            if yy < 0 or xx < 0 or yy + ls > height or xx + ls > width:
                continue
            region = img_gray[yy:yy+ls, xx:xx+ls].astype(np.float64)
            r = region - region.mean()
            t = alpha_flat - alpha_flat.mean()
            denom = np.sqrt(np.sum(r**2) * np.sum(t**2))
            if denom > 0:
                score = np.sum(r * t) / denom
                if score > best_ncc:
                    best_ncc = score
    ...
    if bg_std < 5.0:
        ...
        # 내부(mask≈0): RA 그대로 / 경계(mask≈1): smoothed RA
